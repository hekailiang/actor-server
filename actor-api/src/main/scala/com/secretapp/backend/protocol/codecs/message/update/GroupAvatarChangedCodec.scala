...
object GroupAvatarChangedCodec extends Codec[GroupAvatarChanged] with utils.ProtobufCodec {
  def encode(u: GroupAvatarChanged) = {
    val boxed = protobuf.UpdateGroupAvatarChanged(u.groupId, u.randomId, u.userId,
  ...
  def decode(buf: BitVector) = {
    decodeProtobuf(protobuf.UpdateGroupAvatarChanged.parseFrom(buf.toByteArray)) {
      case Success(protobuf.UpdateGroupAvatarChanged(groupId, randomId, uid, avatar, date)) =>
        GroupAvatarChanged(groupId, randomId, uid, avatar.map(proto.fromProto[models.Avatar, protobuf.Avatar]), date)
    }
  }
}

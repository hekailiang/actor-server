...
object OkCodec extends Codec[Ok] {
  private val rpcResponseMessageCodec: Codec[RpcResponseMessage] = discriminated[RpcResponseMessage].by(uint32)
    ...
    .\(ResponseDialogs.header) { case r: ResponseDialogs => r}(protoPayload(ResponseDialogsCodec))
    ...
    .\(ResponseHistory.header) { case r: ResponseHistory => r}(protoPayload(ResponseHistoryCodec))

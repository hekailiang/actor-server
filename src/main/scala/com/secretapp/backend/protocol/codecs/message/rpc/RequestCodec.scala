...
object RequestCodec extends Codec[Request] {
  val rpcRequestMessageCodec: Codec[RpcRequestMessage] = discriminated[RpcRequestMessage].by(uint32)
    ...
    .\(RequestGetAuth.requestType)             { case r: RequestGetAuth                  => r } (protoPayload(RequestGetAuthCodec))
    .\(RequestLogout.requestType)              { case r: RequestLogout                   => r } (protoPayload(RequestLogoutCodec))
    .\(RequestRemoveAuth.requestType)          { case r: RequestRemoveAuth               => r } (protoPayload(RequestRemoveAuthCodec))
    .\(RequestRemoveAllOtherAuths.requestType) { case r: RequestRemoveAllOtherAuths      => r } (protoPayload(RequestRemoveAllOtherAuthsCodec))
    ...
    .\(SubscribeToGroupOnline.requestType)     { case r: SubscribeToGroupOnline          => r } (protoPayload(SubscribeToGroupOnlineCodec))
    .\(UnsubscribeFromGroupOnline.requestType) { case r: UnsubscribeFromGroupOnline      => r } (protoPayload(UnsubscribeFromGroupOnlineCodec))
    ...
    .\(RequestCreateGroup.requestType)          { case r: RequestCreateGroup               => r } (protoPayload(RequestCreateGroupCodec))
    ...
    .\(RequestLeaveGroup.requestType)           { case r: RequestLeaveGroup                => r } (protoPayload(RequestLeaveGroupCodec))

  ...
  transportForeach { implicit transport =>
    ...
    "sign up" should {
      ...
      "succeed with new public key and same authId" in {
        ...
        AuthSmsCode.insertEntity(models.AuthSmsCode(scope.user.phoneNumber, smsHash, smsCode)).sync()
...
//      }
      ...
      "send ContactRegistered notifications" in {
...

        {
          ...
          UnregisteredContact.insertEntity(models.UnregisteredContact(unregPhone, scope.user.uid)).sync()
...

        {
          ...
          val publicKey = genPublicKey
          insertAuthId(authId)
          AuthSmsCode.insertEntity(models.AuthSmsCode(unregPhone, smsHash, smsCode)).sync()
